addNode("p", "margin: 35px" ,`<br class="leancloud-visitors" data-flag-title="${document.title}" id="${location.pathname}"/>转载请注明原文地址： <a href='${location.href}'>${location.href}</a><small>&nbsp;&nbsp;(&nbsp;创建于：${document.head.querySelector('[name=modifydate]').content}&nbsp;&nbsp;阅读量：<i class="leancloud-visitors-count">--</i>&nbsp;)</small>`);
