...
window.addEventListener("load", () => {
    ...
    let click = (e) => {
        ...
        if (leftPanel.style.width) {
            ...
            leftPanel.style.width = "1em";
            document.body.style.marginLeft = "3em";
        }
    };
    leftPanel.addEventListener("click", click);
    ...
    utils.getAjax("/info.json", json => {
        ...
        Object.values(pacs).forEach(pac => {
            ...
            indexPanel.appendChild(utils.createNode("hr"));
            indexPanel.appendChild(utils.createNode("h3", "", pac.ol_name));
            indexPanel.appendChild(utils.createNode("ol", "", olhtml));
        });
        // 标签列表
        kws.forEach(k => {
            labelPanel.appendChild(utils.createNode('small', '', `<a data-label="${k}" onclick="choiceKeyword('${k}')">${k}</a>`))
        });
    });
});