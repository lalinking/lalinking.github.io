...
function initBookShelf(metaInfo) {
    ...
    let maxHeight = 0;
    let left = 0;
    for (let p in metaInfo) {
        let meta = metaInfo[p];
        meta.key = p;
        let bookNameKey = stringToHashKey(meta.BookName);
        if (bookInfos[bookNameKey]) {
            bookInfos[bookNameKey].contents.push(meta);
        } else {
            bookInfos[bookNameKey] = {BookName: meta.BookName, contents: [meta]};
        }
        maxThickness = Math.max(bookInfos[bookNameKey].contents.length, maxThickness);
        maxHeight = Math.max(bookInfos[bookNameKey].BookName.length, maxHeight);
    }
...
function showPost(filePath, _title, _date) {
    ...
    if (filePath.endsWith(".md")) {
        // markdown 文件
        ajax("posts/" + filePath).then(function(txt) {
            let html = marked(txt, {
                breaks: true,
                smartLists: true,
                smartypants: true,
...
if (!mdContent.length) {
    ajax("/meta_info.json").then(initBookShelf).catch(showNetError);
}
