...
function initStrs() {
    pLis.length = 0;
    let charCanvas = $("#canvas_char")[0];
    let ctx = charCanvas.getContext('2d');
    let chars = toDBC($("#input_char")[0].value);
...
function draw() {
    let imgCanvas = $("#canvas_img")[0];
    let ctx = imgCanvas.getContext('2d');
    ctx.clearRect(0, 0, imgCanvas.width, imgCanvas.height);
    ctx.drawImage($("#img")[0], 0, 0, imgCanvas.width, imgCanvas.height);
...
function copyRes() {
    let msg = $("#msg")[0];
    copyToClipboard($("#output_pic")[0].innerText, () => {
        msg.innerText = "拷贝成功"
    }, err => {
